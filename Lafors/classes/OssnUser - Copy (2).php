...
class OssnUser extends OssnEntities {
		...
		public function getUser() {
			...
			/* 	if(empty($user) && !empty($this->mobile)) {
				...
				if(empty($user) && !empty($this->location)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
				...
				if(empty($user) && !empty($this->designation)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
				...
				if(empty($user) && !empty($this->course)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
								"course='{$this->course}'"
						);
				...
				if(empty($user) && !empty($this->sub_course)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
								"sub_course='{$this->sub_course}'"
						);
						$user             = $this->select($params);
				}
				if(empty($user) && !empty($this->experience)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
				...
				if(empty($user) && !empty($this->expertise)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
	...
	/* 	public function proTypes() {
				...
				return ossn_call_hook('user', 'profession:types', $this, $profession);
		} */
		/**
		 * Get total users per month for each year
		 *
		 ...
		 * 
		 */
	/* 	public function countByPro($profession = 'student') {
				if(!in_array($profession, $this->proTypes())) {
						return false;
				}
				$params                = array();
				$params['type']        = 'user';
				$params['subtype']     = 'profession';
		 ...
		 * 
		 */
		/* public function onlineByPro($profession = 'student', $count = false, $intervals = 100) {
				if(empty($profession) || !in_array($profession, $this->proTypes())) {
						return false;
				}
				$time             = time();
				$params           = array();
				...
				$params['params'] = array(
						'u.*, emd.value as profession'
				);
				$params['from']   = 'ossn_users as u';
				
				$wheres['wheres'][] = "e.type='user'";
				$wheres['wheres'][] = "e.subtype='profession'";
				$wheres['wheres'][] = "emd.value='{$profession}'";
