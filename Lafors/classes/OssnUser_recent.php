...
class OssnUser extends OssnEntities {
		...
		public function getUser() {
				...
				/* if(empty($user) && !empty($this->mobile)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
								"mobile='{$this->mobile}'"
						);
			...
			/* 	if(empty($user) && !empty($this->college)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
				...
				if(empty($user) && !empty($this->profession)) {
						$params['from']   = 'ossn_users';
						$params['wheres'] = array(
		...
		public function Login() {
				...
				if($password == $user->password && $user->activation == NULL) {
					...
					//	OssnSession::assign('OSSN_USER', $user->profession);
						echo $user;
		...
		public function getUserlist(){
				...
				$sql= $this->statement("SELECT DISTINCT profession FROM ossn_users");
				$this->execute($sql);
				$contents['profession'] = $this->fetch(true);
				/* $sql1= $this->statement("SELECT DISTINCT location FROM ossn_users where willing='yes'");
				$this->execute($sql1);
				$contents['location'] = $this->fetch(true); */
				$sql2= $this->statement("select courseid as corid,coursename as corname from  maincourse_master order by coursename asc");
				$this->execute($sql2);
				$contents['course'] = $this->fetch(true);
				$sql3= $this->statement("select subcourseid as subid,courseid as corid,subcoursename as subname from subcourse_master order by subcoursename asc");
				$this->execute($sql3);
				$contents['sub_course'] = $this->fetch(true);
				$sql4= $this->statement("SELECT DISTINCT yearcollege FROM ossn_users where willing='yes'");
				$this->execute($sql4);
				$contents['yearcollege'] = $this->fetch(true);
				$sql5= $this->statement("SELECT year_id as yearid,year as years FROM expyear_master");
				$this->execute($sql5);
				$contents['year'] = $this->fetch(true);
				$sql8= $this->statement("SELECT DISTINCT organization FROM ossn_users where willing='yes'");
				$this->execute($sql8);
				$contents['organization'] = $this->fetch(true);
				$sql10= $this->statement("SELECT DISTINCT college FROM ossn_users where willing='yes'");
				$this->execute($sql10);
				$contents['college'] = $this->fetch(true);
				$sql11= $this->statement("SELECT StateID as stateid,StateName as states FROM statemaster order by states asc");
				$this->execute($sql11);
				$contents['state'] = $this->fetch(true);
				$sql12= $this->statement("SELECT city_name as cities FROM city_master order by cities asc");
				$this->execute($sql12);
				$contents['city'] = $this->fetch(true);
				$sql13= $this->statement("SELECT indname as iname FROM industry_master ");
				$this->execute($sql13);
				$contents['industry'] = $this->fetch(true);
				$sql14= $this->statement("SELECT funname as fname FROM functional_master ");
				$this->execute($sql14);
				$contents['functional'] = $this->fetch(true);
				$sql15= $this->statement("SELECT rolename as rname FROM role_master");
				$this->execute($sql15);
				$contents['role'] = $this->fetch(true);
				$sql16= $this->statement("SELECT year as yr FROM expyear_master");
				$this->execute($sql16);
				$contents['yer'] = $this->fetch(true);
				$sql17= $this->statement("SELECT month as mon FROM expmonth_master");
				$this->execute($sql17);
				$contents['months'] = $this->fetch(true);
				
				$sql18= $this->statement("SELECT * FROM willing_master");
				$this->execute($sql18);
				$contents['human1'] = $this->fetch(true);
				
				$sql19= $this->statement("SELECT * FROM willing_master where search_for='PO'");
				$this->execute($sql19);
				$contents['place'] = $this->fetch(true);
				
				$sql20= $this->statement("SELECT * FROM willing_master where search_for='SME'");
				$this->execute($sql20);
				$contents['sme1'] = $this->fetch(true);
				
				$sql21= $this->statement("SELECT * FROM willing_master where search_for='HR'");
				$this->execute($sql21);
				$contents['human'] = $this->fetch(true);
				
				$sql22= $this->statement("SELECT willing FROM ossn_users  where willing !='yes' && willing!=''");
				$this->execute($sql22);
				$contents['willing'] = $this->fetch(true);
				
				$sql23= $this->statement("SELECT * FROM willing_master where search_for='Student'");
				$this->execute($sql23);
				$contents['stu'] = $this->fetch(true);
...
$sql=$this->statement("SELECT *
FROM user
WHERE willing ='Yes' AND (designation LIKE concat(".$desig.",'%') or ".$desig."= "null")
      AND (location  LIKE concat(".$loc.",'%') or ".$loc."= "null")");
   ...
   $this->execute();
	...
	/**
		...
		/** Edited by sharp
		 * Advance search users using a keyword or entities_pairs
		 *
		 * @param array $params A valid options in format:
		 * 	  'keyword' 		=> A keyword to search users
		 *    'entities_pairs'  => A entities pairs options, must be array
		...
		public function advanceSearch(array $params = array()) {
				if(empty($params)) {
						return false;
				}
				...
				/* if(!empty($options['keyword'])) {
						/*$wheres[] =  "(CONCAT (u.first_name, ' ', u.last_name) LIKE '%{$options['keyword']}%' OR
									  u.username LIKE '%{$options['keyword']}%' OR
									  u.email LIKE '%{$options['keyword']}%' OR
									  u.profession LIKE '%{$options['keyword']}%' )"; 
									   */
									//$sql1= $this->statement("SELECT DISTINCT location FROM ossn_users");
				//$this->execute("call search()");
