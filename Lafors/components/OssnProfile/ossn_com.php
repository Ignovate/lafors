...
function ossn_profile() {
		...
		ossn_add_hook('advancesearch', 'type:users', 'getadvsearch');
		ossn_add_hook('profile', 'subpage', 'profile_user_about');
	...
	ossn_profile_subpage('about');
		...
		if(ossn_isLoggedin()) {
				...
				ossn_register_sections_menu('newsfeed', array(
						'name' => 'friends',
						'text' => ossn_print('user:connects'),
						'url' => $user_loggedin->profileURL('/connects'),
						'parent' => 'links',
						'icon' => $icon
				));
		}
...
function ossn_profile_load_event($event, $type, $params) {
		...
		$url   = ossn_site_url();
		ossn_register_menu_link('about', 'about', $owner->profileURL('/edit'), 'user_timeline');
		ossn_register_menu_link('timeline', 'timeline', $owner->profileURL(), 'user_timeline');
		ossn_register_menu_link('connects', 'connects', $owner->profileURL('/connects'), 'user_timeline');
...
function profile_user_friends($hook, $type, $return, $params) {
		...
		if($page == 'connects') {
				...
				$friends      = ossn_plugin_view('profile/pages/connects', $user);
...
/**
 * Add profile about page
 *
 * Test by sharp
 */

/* function profile_user_about($hook, $type, $return, $params)  {
		$page = $params['subpage'];
		if($page == 'edit') {
				$user['user'] = $params['user'];
...
function profile_search_handler($hook, $type, $return, $params) {
 ...
 function getadvsearch($hook, $type, $return, $params) {
		//print_r($params);
		$Pagination    = new OssnPagination;
	    $users         = new OssnUser;
		$data= $users->advanceSearch(array(
				// /* //'wheres' => "u.profession LIKE '%{$params['pro']}%'" */
				 'wheres' => " concat(u.profession LIKE '%{$params['pro']}%' or u.profession =null )AND concat(u.location LIKE '%{$params['loc']}%' or u.location=null) AND concat(u.willing LIKE '%{$params['will']}%' or u.willing=null) AND  concat(u.course Like '%{$params['cors']}%' or u.course=null) AND concat(u.sub_course LIKE '%{$params['sub']}%' or u.sub_course=null) AND concat(u.yearcollege LIKE '%{$params['year']}%' or u.yearcollege=null) AND concat(u.experience LIKE '%{$params['exp']}%' or u.experience=null) AND concat(u.functional LIKE '%{$params['fun']}%' or u.functional=null) AND concat(u.organization LIKE '%{$params['org']}%' or u.organization=null) AND concat(u.industry LIKE '%{$params['ind']}%' or u.industry=null) AND concat(u.designation LIKE '%{$params['des']}%' or u.designation=null) AND concat(u.college LIKE '%{$params['col']}%' or u.college=null) And concat(concat(u.profession,'',u.location,'',u.city,'',u.course,'',u.email,'',u.mobile,'',u.username,'',u.first_name,'',u.last_name,'',u.college,'',u.sub_course,'',u.experience,'',u.functional,'',u.yearcollege,'',u.organization,'',u.industry)Like '%{$params['filter']}%' or concat(u.profession,'',u.location,'',u.willing,'',u.city,'',u.course,'',u.email,'',u.mobile,'',u.username,'',u.first_name,'',u.last_name,'',u.college,'',u.sub_course,'',u.experience,'',u.functional,'',u.yearcollege,'',u.organization,'',u.industry)=null)"
		));
		
		$count         = $users->advanceSearch(array(
				'wheres' => "concat(u.profession LIKE '%{$params['pro']}%' or u.profession =null)AND concat(u.location LIKE '%{$params['loc']}%' or u.location=null) AND  concat(u.willing LIKE '%{$params['will']}%' or u.willing=null) AND concat(u.course LIKE '%{$params['cors']}%' or u.course=null) AND concat(u.sub_course LIKE '%{$params['sub']}%' or u.sub_course=null) AND concat(u.yearcollege LIKE '%{$params['year']}%' or u.yearcollege=null) AND concat(u.experience LIKE '%{$params['exp']}%' or u.experience=null) AND concat(u.functional LIKE '%{$params['fun']}%' or u.functional=null) AND concat(u.organization LIKE '%{$params['org']}%' or u.organization=null) AND concat(u.industry LIKE '%{$params['ind']}%' or u.industry=null) AND concat(u.college LIKE '%{$params['col']}%' or u.college=null) And concat(concat(u.profession,'',u.location,'',u.city,'',u.course,'',u.email,'',u.mobile,'',u.username,'',u.first_name,'',u.last_name,'',u.college,'',u.sub_course,'',u.experience,'',u.functional,'',u.yearcollege,'',u.organization,'',u.industry)Like '%{$params['filter']}%' or concat(u.profession,'',u.location,'',u.city,'',u.willing,'',u.course,'',u.email,'',u.mobile,'',u.username,'',u.first_name,'',u.last_name,'',u.college,'',u.sub_course,'',u.experience,'',u.functional,'',u.yearcollege,'',u.organization,'',u.industry)=null) ",
				'count' => true
		));
   $user['users'] = $data;
		$search        = ossn_plugin_view('output/users', $user);
