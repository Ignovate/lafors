...
class OssnGroup extends OssnObject {
		 ...
		 * @return bool;
		 */
		public function updateGroup($name, $description, $category, $guid) {
				$data   = array(
						'title',
						'description',
						'category'
		...
		public function searchGroups(array $params=array()) {
			//print_r($params);
			//echo $params[lafors_category];
				$params['from']   = 'ossn_object';
				$params['wheres'] = array(
						"concat(concat(title,'',description)Like '%{$params[q]}%' OR concat(title,'',description)=null) AND category LIKE '%{$params[lafors_category]}%' AND type='user' AND subtype='ossngroup'"
				);
