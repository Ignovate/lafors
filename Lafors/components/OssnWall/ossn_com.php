...
function ossn_wall() {
		...
		ossn_new_external_js('places.min', 'vendors/jquery/places.min.js');
...
function ossn_post_page($pages) {
	...
	break;
						...
						if(isset($pages[1]) && isset($pages[2])) {
								...
								$video = ossn_get_userdata("object/{$pages[1]}/ossnwall/videos/{$pages[2]}");
								//get image file else show error page
								if(is_file($video)) {
										//Image cache on wall post #529
										$filesize = filesize($video);
										header("Content-type: video/mp4/3gp");
										header('Expires: ' . gmdate('D, d M Y H:i:s \G\M\T', strtotime("+6 months")), true);
										header("Pragma: public");
										header("Cache-Control: public");
										header("Content-Length: $filesize");
		...
		if($post && $post instanceof OssnWall) {
				...
				if(isset($post->{'file:wallphoto'})) {
						...
						$image = '';
								...
								if(isset($post->{'file:wallvideo'})) {
						$video = str_replace('ossnwall/videos/', '', $post->{'file:wallvideo'});
				} else {
						$video = '';
				}
